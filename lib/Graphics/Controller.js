...
import { isPackaged, translateShowStatusBarFromBoolean } from '../Resources/Util.js'
...
class GraphicsController extends CoreBase {
	...
	constructor(registry) {
		...
		this.#drawOptions = {
			...
			show_statusbar: this.userconfig.getKey('show_statusbar'),
		}
					...
					if (location && locationIsInBounds && buttonStyle && buttonStyle.style) {
						...
						if (buttonStyle.style === 'button') {
							const globalShowTopBar =
								this.#drawOptions.show_statusbar !== 'hidden' && buttonStyle.show_statusbar === 'default'
							keyLocation =
								globalShowTopBar || buttonStyle.show_statusbar === 'top' || buttonStyle.show_statusbar === 'bottom'
									? location
									: undefined
						}
	...
	async drawPreview(buttonStyle) {
		...
		const drawStyle = {
			...
			show_statusbar: translateShowStatusBarFromBoolean(buttonStyle.show_topbar),
	...
	updateUserConfig(key, value) {
		if (key == 'page_direction_flipped') {
			...
			// @ts-expect-error casting string to more specific type
			this.#drawOptions.show_statusbar = String(value)
			this.logger.silly('Status bar changed')
