...
export default class GraphicsRenderer {
	...
	static drawBlank(options, location) {
		...
		if (options.show_statusbar !== 'hidden') {
	...
	static async #drawButtonMain(img, options, drawStyle, location) {
		let statusbar_position = options.show_statusbar
		if (drawStyle.show_statusbar !== 'default') {
			statusbar_position = drawStyle.show_statusbar
		}

		const show_topbar = statusbar_position !== 'hidden'
		const statusbar_y = statusbar_position === 'bottom' ? 58 : 0
		const main_y = statusbar_position === 'top' ? 14 : 0
		const main_height = statusbar_position === 'hidden' ? 72 : 58
		...
		img.box(0, main_y, 72, main_y + main_height, parseColor(drawStyle.bgcolor))
		...
		if (drawStyle.png64 !== undefined && drawStyle.png64 !== null) {
			try {
				...
				await img.drawFromPNGdata(data, 0, main_y, 72, main_height, halign, valign, 'fit')
			} catch (e) {
				console.error('error drawing image:', e)
				img.fillColor('black')
				img.drawAlignedText(2, main_y + 2, 68, main_height - 4, 'PNG ERROR', 'red', 10, 'center', 'center')

				GraphicsRenderer.#drawTopbar(img, show_topbar, statusbar_y, drawStyle, location)
				return
			}
		}
		...
		try {
			for (const image of drawStyle.imageBuffers || []) {
				if (image.buffer) {
					const x = image.x ?? 0
					const y = main_y + (image.y ?? 0)
					const width = image.width || 72
					const height = image.height || main_height
			...
			img.fillColor('black')
			img.drawAlignedText(2, main_y + 2, 68, main_height - 4, 'IMAGE\\nDRAW\\nERROR', 'red', 10, 'center', 'center')
	 ...
	 * @param {number} statusbar_y
	 * @param {import('../Shared/Model/StyleModel.js').DrawStyleButtonModel} drawStyle The style to draw
	 * @param {import('../Resources/Util.js').ControlLocation | undefined} location
	 * @access private
	 */
	static #drawTopbar(img, show_topbar, statusbar_y, drawStyle, location) {
		if (!show_topbar) {
			if (drawStyle.pushed) {
			...
			let step = ''
			img.box(0, statusbar_y, 72, statusbar_y + 14, colorBlack)
			img.horizontalLine(statusbar_y == 0 ? 13.5 : statusbar_y - 0.5, colorButtonYellow)
			...
			if (location === undefined) {
				// Preview (no location)
				img.drawTextLine(4, statusbar_y + 2, `x.x${step}`, colorButtonYellow, 9)
			} else if (drawStyle.pushed) {
				img.box(0, statusbar_y, 72, statusbar_y + 14, colorButtonYellow)
				img.drawTextLine(4, statusbar_y + 2, `${formatLocation(location)}${step}`, colorBlack, 9)
			} else {
				img.drawTextLine(4, statusbar_y + 2, `${formatLocation(location)}${step}`, colorButtonYellow, 9)
			}
		}
		...
		if (drawStyle.cloud && show_topbar) {
			img.drawPixelBuffer(rightMax - 17, statusbar_y + 3, 15, 8, internalIcons.cloud)
			rightMax -= 17
		}
		...
		if (location) {
			switch (drawStyle.button_status) {
				case 'error':
					img.box(rightMax - 10, statusbar_y + 3, rightMax - 2, statusbar_y + 11, 'red')
					rightMax -= 10
					break
				case 'warning':
					img.drawFilledPath(
						[
							[rightMax - 10, statusbar_y + 11],
							[rightMax - 2, statusbar_y + 11],
							[rightMax - 6, statusbar_y + 3],
						],
						'rgb(255, 127, 0)'
					)
					img.drawTextLineAligned(rightMax - 6, statusbar_y + 11, '!', colorBlack, 7, 'center', 'bottom')
					rightMax -= 10
					break
			}
			...
			if (drawStyle.action_running) {
				//img.drawTextLine(55, statusbar_y+3, '►', 'rgb(0, 255, 0)', 8) // not as nice
				let iconcolor = 'rgb(0, 255, 0)'
				if (drawStyle.pushed) iconcolor = colorBlack
				img.drawFilledPath(
					[
						[rightMax - 8, statusbar_y + 3],
						[rightMax - 2, statusbar_y + 7],
						[rightMax - 8, statusbar_y + 11],
					],
					iconcolor
				)
				rightMax -= 8
