...
class Image {
	...
	box(x1, y1, x2, y2, color = undefined, strokeColor = undefined, lineWidth = 1, lineOrientation = 'inside') {
		...
		if (strokeColor) {
			didDraw = didDraw || boxLine(x1, y1, x2, y2, strokeColor, lineWidth, lineOrientation)
		}

		if (didDraw) this.lastUpdate = Date.now()
		return didDraw
	}
	...
	drawFromPNGdata(
		...
		try {
			...
			this.logger.error('new ImageData failed', error)
		}
	...
	/**
	 * Draws an image to the canvas from SVG data
	 * the image can be fitted and cropped to the canvas in many variations
	 * @param {string} data plain or base64 encoded string with the SVG XML data
	 * @param {number} xStart left position where to place the image
	 * @param {number} yStart top position where to place the image
	 * @param {number} width width of the bounding box where to place the image
	 * @param {number} height height of the bounding box where to place the image
	...
	drawFromSVGdata(
		...
		if (base64regex.test(data)) {
			try {
				...
				this.logger.error('drawing SVG image failed: image data seems to be base64 encoded, but decoding failed')
				return
			}
		} else if (data.search(/^<\?xml/i) != -1) {
			svgString = data
		} else {
			this.logger.error('drawing SVG image failed: image data seems not to be valid XML format')
			return
		}

		let svgCanvas
		try {
			svgCanvas = Canvg.fromString(imageContext2d, svgString, preset)
			svgCanvas.render()
		} catch (error) {
			this.logger.error('drawing SVG image failed: '+ error)
			return
		}
		
		let imageWidth = imageCanvas.width
		let imageHeight = imageCanvas.height
		...
		try {
			...
			this.logger.error('drawing SVG image failed: '+ error.stack)
			return
		}
	 ...
	 * @param {boolean|undefined} dummy Don't actually draw anything just return the text width
	 ...
	 * @param {string} color CSS color string
	 * @param {number | string} fontsize height of font, either pixels or 'auto'
	...
	drawAlignedText(
		...
		this.context2d.font = `${fontheight}px/${lineheight}px ${fontfamily}`
	 ...
	 * @param {close} close if true close the path from last point to first point, if last and first point are identical path will be autoclosed
	 * @returns {boolean} success
	 */
