...
import { translateShowStatusBarFromBoolean, translateStyleFromModule } from '../Resources/Util.js'
	...
	importPresetToLocation(connectionId, presetId, location) {
		...
		const result = {
			...
			style: {
				...
				show_statusbar: translateShowStatusBarFromBoolean(definition.style.show_topbar),
			},
			feedbacks: [],
			steps: {},
		}
		...
		if (definition.feedbacks) {
			result.feedbacks = definition.feedbacks.map((feedback) => ({
				...
				style: translateStyleFromModule(cloneDeep(feedback.style)),
			}))
		}
